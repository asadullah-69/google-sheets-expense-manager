...
export default function Home() {
  ...
  const loadUserSheets = async () => {
    try {
      const savedSheets = localStorage.getItem(
        `user_sheets_${session?.user?.email}`
      );
      if (savedSheets) {
        const parsedSheets = JSON.parse(savedSheets);
        const validSheets = parsedSheets.filter(
          (sheet: any) =>
            sheet &&
            sheet.id &&
  ...
  const saveUserSheet = (sheetId: string, sheetName: string) => {
    ...
    if (!userEmail || !sheetId || typeof sheetId !== "string" || !sheetName)
      return;
  ...
  const handleDeleteSheet = () => {
    ...
    const updatedSheets = userSheets.filter(
      (sheet) => sheet.id !== deleteModal.sheetId
    );
    setUserSheets(updatedSheets);
    localStorage.setItem(
      `user_sheets_${userEmail}`,
      ...
      <div className="min-h-screen bg-gradient-to-b from-gray-50 to-white">
        ...
        <section className="container mx-auto px-4 py-16 text-center">
          ...
          <p className="text-xl text-gray-600 mb-8 max-w-2xl mx-auto">
            A powerful expense tracking solution that seamlessly syncs with
            Google Sheets. Keep your finances organized and accessible from
            anywhere.
          </p>
          <button
            onClick={() => signIn("google")}
            ...
            {[
              {
                ...
                description:
                  "Add, edit, and delete expenses with a user-friendly interface",
                icon: (
                  <svg
                    className="w-6 h-6"
                    fill="none"
              ...
              },
              {
                ...
                description:
                  "Automatically sync your expenses with Google Sheets",
                icon: (
                  <svg
                    className="w-6 h-6"
                    fill="none"
              ...
              },
              {
                title: "Smart Search",
                description:
                  "Quickly find expenses with powerful search capabilities",
                icon: (
                  <svg
                    className="w-6 h-6"
                    fill="none"
              ...
              <div className="border-2 border-dashed border-gray-300 rounded-lg p-6 hover:border-blue-500 transition-colors">
                <div className="text-center">
                  ...
                  <p className="text-gray-600 mb-4">
                    Start fresh with a new expense tracking template
                  </p>
                  <button
                ...
                <div className="text-center">
                  ...
                  {userSheets.length > 0 ? (
                      ...
                      <div className="space-y-2 mb-4">
                        {userSheets.map((sheet) => (
                          ...
                          >
                            <button
                              onClick={() =>
                                handleSelectExistingSheet(sheet.id)
                              }
                              className="flex-1 text-left"
                              disabled={
                                !sheet.id || typeof sheet.id !== "string"
                              }
                            >
                            ...
                            <button
                              onClick={() =>
                                openDeleteModal(sheet.id, sheet.name)
                              }
                    ...
                    <p className="text-gray-500 mb-4">
                      No existing spreadsheets found
                    </p>
                  )}
                </div>
              </div>
            ...
            <div className="bg-white rounded-lg shadow-xl max-w-md w-full border-2 border-gray-200">
              ...
              <div className="p-6">
                ...
                <p className="text-gray-600 text-center mb-2">
                  Are you sure you want to remove this spreadsheet from your
                  list?
                </p>
                <p className="text-sm text-gray-500 text-center mb-4">
                  This will remove "
                  <span className="font-medium">{deleteModal.sheetName}</span>"
                  from your list.
                  <br />
